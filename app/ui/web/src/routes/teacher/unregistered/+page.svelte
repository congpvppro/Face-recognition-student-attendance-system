<script lang="ts">
  ...
  let {
    ...
    data: {
      unregisteredFaces: UnregisteredFace[];
      students: Student[];
      classes: Class[];
    };
  } = $props();
          ...
          <figure class="relative bg-base-300">
            ...
            {:else}
              <img
                ...
                loading="lazy"
                onerror={() => imageErrors.set(face.face_id, true)}
              />
            {/if}
              ...
              {:else}
                ...
                >
                  ...
                  <div>
                    ...
                    <select
                      name="classId"
                      id="class-{face.face_id}"
                      class="select select-bordered w-full"
                      required
                      disabled={loading("create")}
                    >
                      <option value={face.class_id} selected>
                        {data.classes.find((c) => c.id === face.class_id)
                          ?.name || `Class ID: ${face.class_id}`}
                      </option>
                      {#each data.classes as cls}
                        {#if cls.id !== face.class_id}
